...
def get_contribution_data(
    ...
    for year in years:
        ...
        for line in contributions:
            ...
            if not match:
                continue
            contribution_date = match.group(1).decode()
            if start_date and contribution_date < start_date:
                continue
            if end_date and contribution_date > end_date:
                continue
            if match.group(2) != b'0':
                logger.debug("date=%s count=%s", match.group(1), match.group(2))
                contribution_data[contribution_date] = int(match.group(2))
...
def validate_date(date_value: str) -> str:
    ...
    try:
        ...
        raise argparse.ArgumentTypeError(
            '{} is not a valid date in YYYY-MM-DD format'.format(date_value))
    return date_value


def get_years_in_range(start_date: str, end_date: str) -> List[bytes]:
    """Return all years touched by the configured date range."""

    start_year = datetime.strptime(start_date, '%Y-%m-%d').year
    end_year = datetime.strptime(end_date, '%Y-%m-%d').year
    return [str(year).encode() for year in range(start_year, end_year + 1)]
...
def cli_get_configuration(args: argparse.Namespace) -> Dict[str, str]:
    """Read configuration from the command line"""

    config: Dict[str, str] = {}

    config['username'] = args.username or input('Your GitHub username: ')
    if not config['username']:
        logger.error('Username required')
        sys.exit(1)
    config['email'] = args.email or input('Your GitHub email address: ')
    if not config['email']:
    ...
    config['password'] = ''
    if not args.ssh:
        config['password'] = os.environ.get('GITHUB_TOKEN', '')
    if not args.ssh and not config['password']:
    ...
    if not args.ssh and not config['password']:
        ...
        sys.exit(1)
    config['repo'] = args.repo or input('GitHub repository to create commits in: ')
    if not config['repo']:
        logger.error('Repository name required')
        sys.exit(1)
    config['donor'] = args.donor or input('GitHub user to clone: ')
    if not config['donor']:
...
def run_git(args: List[str], cwd: Optional[str] = None, env: Optional[Dict[str, str]] = None) -> None:
    """Run git and raise a useful error if it fails."""
...
def create_and_push_with_git(
        config: Dict[str, str],
        repo_url: str,
        repo_tmpdir: str,
        repo_data_file: str,
        data_repo: Dict[str, int]) -> None:
    """Create and push backdated commits using the installed git CLI."""

    print('Cloning {}...'.format(repo_url))
    run_git(['clone', repo_url, repo_tmpdir])
    run_git(['config', 'user.name', config['username']], cwd=repo_tmpdir)
    run_git(['config', 'user.email', config['email']], cwd=repo_tmpdir)
    ...
    for (commit_date, commit_count) in data_repo.items():
        for commit_num in range(0, commit_count):
            ...
            commit_date_iso = datetime.fromtimestamp(
                commit_stamp,
            ...
            with open(repo_data_file, 'w') as data_file:
                data_file.write(commit_date + str(commit_num))

            run_git(['add', config['data_file']], cwd=repo_tmpdir, env=env)
            run_git([
                'commit',
                '--author',
                '{} <{}>'.format(config['username'], config['email']),
                '-m',
                commit_date
                ], cwd=repo_tmpdir, env=env)
            commits_generated += 1
            ...
            if not commits_generated % COMMITS_PER_PUSH:
                run_git(['push', 'origin', 'HEAD:main'], cwd=repo_tmpdir)
...
def main() -> None:
    ...
    parser.add_argument('--ssh', action='store_true', help='Use SSH for clone and push')
    ...
    config: Dict[str, str] = cli_get_configuration(args)

    tempdir = tempfile.TemporaryDirectory()
    logger.info('Using tempdir=%s', tempdir.name)

    if args.ssh:
        repo_url = 'git@github.com:' +  config['username'] + '/' + config['repo'] + '.git'
    else:
    ...
    if args.start_date and args.end_date:
        active_years: List[bytes] = get_years_in_range(args.start_date, args.end_date)
    else:
    ...
    data_user: Dict[str, int] = get_contribution_data(
            config['username'],
            active_years,
            args.start_date,
            args.end_date)
    print('Getting activity for {}...'.format(config['donor']))
    data_donor: Dict[str, int] = get_contribution_data(
            config['donor'],
            active_years,
            args.start_date,
            args.end_date)
